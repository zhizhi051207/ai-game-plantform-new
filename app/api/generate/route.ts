...
export async function POST(request: NextRequest) {
  try {
    ...
    // 调用OpenRouter Meta Llama 3.1 8B Instruct（通过friendli托管）生成HTML游戏
    console.log("Calling Meta Llama 3.1 8B Instruct (via friendli) with prompt:", prompt.substring(0, 100));
    const completion = await openai.chat.completions.create({
      model: "friendli/meta-llama/llama-3.1-8b-instruct", // 使用用户指定的Meta Llama 3.1 8B Instruct模型，通过friendli托管
      messages: [
        {
          role: "system",
...
6. Responsive design that works on desktop and mobile`,
    ...
    if (error.code === 403) {
      ...
      errorMessage = "Model configuration error. Please check if Meta Llama 3.1 8B Instruct (friendli/meta-llama/llama-3.1-8b-instruct) is available on OpenRouter.";
    }
